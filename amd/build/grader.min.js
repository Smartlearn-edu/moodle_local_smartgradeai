define(['jquery', 'core/ajax', 'core/notification', 'core/str'], function ($, Ajax, Notification, Str) {
    return {
        init: function (params) {
            ...
            console.log('Autograde Helper: Init called with params:', params, 'type:', typeof params);
            ...
            $(document).ready(function () {
                ...
                console.log('Autograde Helper: Checking .submissionstatustable', container.length);

                if (container.length === 0) {
                    container = $('.gradingtable').first();
                    console.log('Autograde Helper: Checking .gradingtable', container.length);
                }

                if (container.length === 0) {
                    // Try finding the grading actions form or region
                    container = $('.grading-actions-form');
                    console.log('Autograde Helper: Checking .grading-actions-form', container.length);
                }

                if (container.length === 0) {
                    container = $('[role="main"]');
                    console.log('Autograde Helper: Checking [role="main"]', container.length);
                }
                ...
                Str.get_string('grade_with_ai_button', 'local_autogradehelper').done(function (buttonLabel) {
                    ...
                    button.click(function (e) {
                        ...
                        console.log('Autograde Helper: Button clicked');
                        ...
                        promises[0].done(function (response) {
                            console.log('Autograde Helper: Response', response);
                            if (response.success) {
                                Str.get_string('trigger_success', 'local_autogradehelper').done(function (msg) {
                                    Notification.alert('Success', msg, 'Ok');
                                });
                            } else {
                                Str.get_string('trigger_error', 'local_autogradehelper', response.message).done(function (msg) {
                                    Notification.alert('Error', msg, 'Ok');
                                });
                            }
                        }).fail(function (ex) {
                            console.error('Autograde Helper: Failed', ex);
                    ...
                    console.log('Autograde Helper: Button appended');
                });
            });
        }
    };
